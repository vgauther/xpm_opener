...
/*   Updated: 2019/11/20 19:10:35 by vgauther         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	begin_check_contruct_of_color(char **tmp, int nb_char_for_pix)
{
...
int			construction_of_color_line(char *str, int nb_char_for_pix)
{
	...
	if ((i = 0) == 0 && (int)ft_strlen(str) < nb_char_for_pix)
		return (1);
	if (!(tmp = ft_strsplit(str, '"')))
		return (1);
	if (begin_check_contruct_of_color(tmp, nb_char_for_pix))
		return (xpm_free_tab_char_with_ret_1(tmp));
